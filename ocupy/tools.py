...
def spline(ad,ld,collapse=True,xdim=[-36,36]):
	...
	if collapse: # von 0 bis 181
		...
		ad = abs(ad)
		K = createHist(ld,ad)
		H = spline_base.spline_pdf(np.array(samples), e_y, e_x, nr_knots_y = 3, nr_knots_x = 19,hist=K)		
		...
		H = spline_base.spline_pdf(np.array(samples), e_y, e_x, nr_knots_y = 3, nr_knots_x = 19)
	return H/H.sum()